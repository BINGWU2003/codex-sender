...
“成功交接后清空 Cursor 输入框”是独立的全局开关，默认关闭。开启后，工具会在跳转前再次核对 Cursor 当前选中的完整内容；只有与刚复制的提示词完全一致时，才通过 `Ctrl+A`、`Delete` 清空，核对失败则保留原内容并继续交接。
...
默认情况下，读取完成后工具会恢复原来的焦点、光标和选区；启用“成功交接后清空 Cursor 输入框”后则清除本次提示词。富文本包含文件节点但复制结果缺少 `@` 时会停止交接，不会静默发送丢失路径的文本。
...
- 交接方式是全局设置，对所有工作区生效。
- “成功交接后清空 Cursor 输入框”可独立开关，默认关闭并对所有工作区生效。
